"""
MODIFIED VERSION: 20200610 rwbl
IN PROGRESS!!!
...
def error(*args):
	domoticz_.Log("ERROR".join([str(k) for k in args]))
...
class Device(object):
    ...
    # updatedevice nvalue & svalue
    # IN PROGRESS - not working yet, i.e. 
    # the function is not recognised and the device is not updated.
    # Need to explore how this works.
    def updatedevice(self, nvalue, svalue, after=None, reflect=False):
        self.n_value = nvalue
        self.s_value = svalue
        log("updatedevice:", svalue)

    # Same applies for the update command
    # i.e. what commend to use to f.e. update a text device
    # event_system.command(name, action, file)
